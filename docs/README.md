# Splendor Documentation

This directory keeps the core product contracts at the top level and groups supporting material by
workflow.

## Core contracts

- [Product spec](splendor_product_spec.md)
- [Roadmap](splendor_mvp_to_v1_roadmap.md)
- [Schema contracts](schema_contracts.md)
- [Source-resolution refactor plan](source_resolution_refactor_plan.md)
- [Issue #70 design response](issue_70_design_response.md)

## Guides

- [Quickstart](guides/quickstart.md)
- [Companion repo setup](guides/companion_repo_setup.md)
- [Dogfooding](guides/dogfooding.md)

## Operations

- [CI and repo automation](operations/ci_and_repo_automation.md)
- [GitHub automation architecture](operations/github_automation_architecture.md)
- [Release artifacts](operations/release_artifacts.md)

## Evaluations

- [Dogfood knowledge-work report](evaluations/dogfood_knowledge_work_report.md)
- [M14 SynthBanshee reevaluation](evaluations/m14_synthbanshee_reevaluation.md)
- [v0.2 SynthBanshee evaluation](evaluations/v0_2_synthbanshee_evaluation.md)
- [v0.3 SynthBanshee evaluation](evaluations/v0_3_synthbanshee_evaluation.md)
- [v0.3 SynthBanshee follow-up](evaluations/v0_3_synthbanshee_followup.md)
- [v0.3 hocrgen evaluation](evaluations/v0_3_hocrgen_evaluation.md)
- [v0.3 hocrgen follow-up](evaluations/v0_3_hocrgen_followup.md)
- [v0.4 external retry bar](evaluations/v0_4_external_retry_bar.md)
- [Public mock client acceptance](evaluations/public_mock_client_acceptance.md)

## Releases

- [v0.2.0 release notes](releases/v0_2_0_release_notes.md)
- [v0.3.0 release notes](releases/v0_3_0_release_notes.md)
- [v0.4.0 release notes](releases/v0_4_0_release_notes.md)
- [v1 release handoff](releases/v1_release_handoff.md)
