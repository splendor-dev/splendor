---
schema_version: '1'
kind: source-summary
title: round 1 llm wiki implementation landscape
page_id: src-6be68f3b5ee70bf209d78171363cbd857c72f331dbdd13ddc79fd2f7d8c188a6
status: active
review_state: machine-generated
source_refs:
- src-6be68f3b5ee70bf209d78171363cbd857c72f331dbdd13ddc79fd2f7d8c188a6
generated_by_run_ids:
- run-src-6be68f3b5ee70bf209d78171363cbd857c72f331dbdd13ddc79fd2f7d8c188a6-20260430T092101207478Z
last_generated_at: '2026-04-30T09:21:01+00:00'
last_reviewed_at: null
confidence: 1.0
related_pages: []
tags:
- source-summary
- md
provenance_links:
- source_id: src-6be68f3b5ee70bf209d78171363cbd857c72f331dbdd13ddc79fd2f7d8c188a6
  page_id: null
  run_id: null
  path_ref: state/manifests/sources/src-6be68f3b5ee70bf209d78171363cbd857c72f331dbdd13ddc79fd2f7d8c188a6.json
  role: generated-from
  note: null
- source_id: null
  page_id: null
  run_id: run-src-6be68f3b5ee70bf209d78171363cbd857c72f331dbdd13ddc79fd2f7d8c188a6-20260430T092101207478Z
  path_ref: null
  role: generated-from
  note: null
- source_id: null
  page_id: null
  run_id: null
  path_ref: raw/imports/dogfood-knowledge-work/round-1-llm-wiki-implementation-landscape.md
  role: input
  note: null
contradictions: []
---

# round 1 llm wiki implementation landscape

## Source

- Source ID: `src-6be68f3b5ee70bf209d78171363cbd857c72f331dbdd13ddc79fd2f7d8c188a6`
- Source type: `md`
- Registered path: `raw/imports/dogfood-knowledge-work/round-1-llm-wiki-implementation-landscape.md`
- Source file: `raw/imports/dogfood-knowledge-work/round-1-llm-wiki-implementation-landscape.md`

## Summary

This page records deterministic ingestion output for source `src-6be68f3b5ee70bf209d78171363cbd857c72f331dbdd13ddc79fd2f7d8c188a6`, a `md` file registered from `raw/imports/dogfood-knowledge-work/round-1-llm-wiki-implementation-landscape.md`.

## Key Facts

- Source ID: `src-6be68f3b5ee70bf209d78171363cbd857c72f331dbdd13ddc79fd2f7d8c188a6`
- Source type: `md`
- Checksum: `6be68f3b5ee70bf209d78171363cbd857c72f331dbdd13ddc79fd2f7d8c188a6`
- Source ref: `raw/imports/dogfood-knowledge-work/round-1-llm-wiki-implementation-landscape.md`
- Added at: `2026-04-30T09:20:51+00:00`
- Ingested at: `2026-04-30T09:21:01+00:00`
- The LLM Wiki pattern is no longer only an idea-file pattern. It has become a fast-moving product category with local apps, MCP servers, wiki compilers, hosted variants, and commercial RAG vendors reframing their roadmaps around compiled knowledge.
- The common shape is stable: source files remain durable, a maintained markdown wiki becomes the working knowledge layer, and agent instructions or schemas define the maintenance contract.
- Implementations differ most strongly on where they place complexity: hidden local indexes, semantic search, richer web UI, MCP surfaces, typed schemas, review queues, and multimodal extraction.

## Extract

```text
# Round 1: LLM Wiki Implementation Landscape

Source URLs:

- https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f
- https://github.com/lucasastorian/llmwiki
- https://denser.ai/blog/llm-wiki-karpathy-knowledge-base/

## Source Note

This note expands Splendor's existing LLM Wiki seed material after a first dogfood pass through the
current repository wiki. The strongest existing wiki pages already captured the raw/written-wiki
split, but they under-described the emerging product landscape around the pattern and how quickly
implementations are converging on agent-facing tools.

## Core Claims

- The LLM Wiki pattern is no longer only an idea-file pattern. It has become a fast-moving product
  category with local apps, MCP servers, wiki compilers, hosted variants, and commercial RAG vendors
  reframing their roadmaps around compiled knowledge.
- The common shape is stable: source files remain durable, a maintained markdown wiki becomes the
  working knowledge layer, and agent instructions or schemas define the maintenance contract.
- Implementations differ most strongly on where they place complexity: hidden local indexes,
  semantic search, richer web UI, MCP surfaces, typed schemas, review queues, and multimodal
  extraction.
- Splendor's distinctive position is its conservative file-contract bias: git-visible manifests,
  queue records, run records, lint reports, planning objects, and deterministic commands before
  hidden indexing or rich UI.
- The market signal is not that Splendor should copy a fuller app immediately. The signal is that
  humans and agents expect a full loop: ingest, compile/update pages, query, file answers, lint, and
  inspect operational state.

## Design Implications

Splendor should keep the raw-source, wiki, and instruction-file triangle as a first-class concept,
but the word "ingest" currently hides two different jobs:

1. Register and summarize a source.
2. Compile that source into existing concept, topic, comparison, and architecture pages.

Current Splendor ingest performs the first job deterministically. Dogfooding shows that the second
job remains too manual for sustained knowledge work. The roadmap should add an explicit compile or
update step rather than overloading deterministic source-summary generation.

## Product Experience Notes

Reading the current wiki before adding this source was genuinely useful: I could learn the product
premise, see prior source summaries, and query for dogfood findings without reading the entire repo.
The weak point was follow-through. After finding a new source, I had to decide manually where
analysis should live, how to update synthesis pages, and how to keep index/log/provenance coherent.

The CLI makes source registration and ingestion feel safe. The missing user affordance is an
opinionated "continue the knowledge loop" command that says what pages should change next.
```

## Provenance

- Manifest: `state/manifests/sources/src-6be68f3b5ee70bf209d78171363cbd857c72f331dbdd13ddc79fd2f7d8c188a6.json`
- Workspace source: `raw/imports/dogfood-knowledge-work/round-1-llm-wiki-implementation-landscape.md`
- Run ID: `run-src-6be68f3b5ee70bf209d78171363cbd857c72f331dbdd13ddc79fd2f7d8c188a6-20260430T092101207478Z`
- Pipeline version: `0.1.0a0`
